...
export default function Frontend({ foods, services, allItems, orders, setOrders, setFoods, setServices }) {
  ...
  {qr ? (
    ...
    "Loading..."
        ...
        <div className="grid">
          {foods.map((item) => (
            <div className="card" key={item.id}>
              ...
              <p>Còn: {item.stock}</p>
              <div className="qty">
                <button onClick={() => setQuantity(item.id, (cart[item.id] || 0) - 1)}>-</button>
                <b>{cart[item.id] || 0}</b>
                <button onClick={() => setQuantity(item.id, (cart[item.id] || 0) + 1)}>+</button>
              </div>
            </div>
      ...
      <aside className="cart">
        ...
        <input placeholder="Tên khách hàng" value={customer.name} onChange={(e) => setCustomer({ ...customer, name: e.target.value })} />
        <input placeholder="Số điện thoại" value={customer.phone} onChange={(e) => setCustomer({ ...customer, phone: e.target.value })} />
        <textarea placeholder="Ghi chú: điểm đón, giờ đi..." value={customer.note} onChange={(e) => setCustomer({ ...customer, note: e.target.value })} />
        ...
        <button className="primary" onClick={submitOrder}>Đặt đơn</button>
